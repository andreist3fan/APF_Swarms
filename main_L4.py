...
def run_sim(setup):
    ...
    while not setup.target and running: 
        ...
        if steps > setup.step_limit: 
            running = False
            logger.log("Run took to long and was stopped.")
...
if __name__ == "__main__":
    ...
    for obs_num in obs_nums:
        ...
        for i in range(trials):
            ...
            logger.log(f"Running simulation #{i+1}/500 with {obs_num} obstacles.")
            run_sim(setup)
            comm_dists[obs_num].append(setup.min_communication_distance)
            logger.log(f"Trial finished.")
    ...
    for obs_num in obs_nums:
        plt.hist(comm_dists[obs_num], bins=30, edgecolor ="black", density=True, label=f"{obs_num} obstacles")
